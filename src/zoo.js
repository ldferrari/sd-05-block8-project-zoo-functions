...
function employeeByName(employeeName) {
  ...
  return (
    employees.find((el) => el.firstName == employeeName) ||
    employees.find((el) => el.lastName == employeeName)
  );
}
...
function isManager(id) {
  ...
  employees.forEach((el) =>
    el.managers.forEach((ell) => (ell == id ? (valor = true) : false))
  );
  return valor;
}
...
function animalCount(species) {
  ...
  return (retorno[species] = animals.find(
    (el) => el.name == species
  ).residents.length);
}
...
function schedule(dayName) {
  ...
  hoursArr.forEach((dday) => {
    ...
    if (el.open == el.close) {
      ...
      workingHours[dday] = `Open from ${el.open}am until ${
        el.close > 12 ? el.close - 12 : el.close
      }${el.close > 12 ? "pm" : "am"}`;
    }
  });
...
function listOfAnimals(keysToUse, el) {
  const resp = {};
  resp[keysToUse] = [];
  el.responsibleFor.forEach((ell) =>
    resp[keysToUse].push(animals.find((an) => an.id == ell).name)
  );
  return resp;
}
